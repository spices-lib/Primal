...
namespace primal::utl {
	...
	public:
		...
		{
			static_assert(std::is_default_constructible_v<T>, "Type must be default-constructrible");
		...
		{
			static_assert(std::is_copy_constructible_v<T>, "Type must be copy-constructrible");
	...
	private:
		...
		{
			assert([&]() -> bool { return _capacity ? _data : !_data; }());
			clear();
			_capacity = 0;
			if (_data) free(_data);
			_data = nullptr;
