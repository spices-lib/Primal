...
namespace primal::graphics::d3d12::upload {
	...
	namespace {
		...
		struct upload_frame
		{
			...
			id3d12_graphic_command_list*  cmd_list{ nullptr };
	...
	void d3d12_upload_context::end_upload()
	{
		assert(_frame_index != u32_invalid_id);
		upload_frame& frame{ upload_frames[_frame_index] };
		id3d12_graphic_command_list* const cmd_list{ frame.cmd_list };
	...
	bool initialize()
	{
		...
		if (FAILED(hr)) return init_failed();
		NAME_D3D12_OBJECT(upload_fence, L"Upload Copy Fence");
