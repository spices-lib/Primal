...
namespace primal::graphics::d3d12::core {
	...
	namespace {
		...
		public:
			...
			constexpr id3d12_graphic_command_list* const command_list() const { return _cmd_list; }
		...
		// Note: this function can be expaned in functionality with, for example, checking if any
	...
	{
		std::unique_lock lock(deferred_releases_mutex);
	...
	bool initialize()
	{
		...
		// Enable debugging layer. Requires "Graphic Tools" optional feature
		{
			...
			{
				OutputDebugStringA("Warning: D3D12 Debug interface is not available. Vertify that Graphics Tools optional feature is installed in this device.");
			}
			debug_interface->EnableDebugLayer();
	...
	void render_surface(surface_id id)
	{
		...
		id3d12_graphic_command_list* cmd_list{ gfx_command.command_list() };
