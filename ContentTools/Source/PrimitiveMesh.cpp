...
namespace primal::tools {
	...
	namespace {
			...
			v3 offset = { -0.5f, 0.0f, -0.5f }, v2 u_range = { 0.0f, 1.0f }, v2 v_range = { 0.0f, 1.0f })
		{
			...
			{
				for (u32 i{ 0 }; i <= horizontal_step; ++i)
				{
					v3 position{ offset };
		...
		mesh create_uv_sphere(const primitive_init_info& info)
		{
			...
			// Indices for the top cap, connecting the north pole to the ring
			...
			// Indices for thr bottom cap, connecting the south posle to the last ring
